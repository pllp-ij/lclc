...
/*
    ...
    DESCRIPTION:
        ...
        STEP 8
        Initialize resultList to new ArrayList to collect all list under iterated key with List<Integer> type
        ...
        Iterate each index of cacheArr from (cacheArr.length - 1) to 0 with idx
            ...
            If k == 0, (meaning the top k frequent elements have all been added to result, there is no need to iterate anymore)
                ...
                Break or return the loop to finish
                    break;
            STEP 14
            Add elements with hashFreqToValList.get(idx + minFreqVal[0]) according the relative value of k and list size by calling method and assign the result to updatedK
        ...
        STEP 16
        Initialize result to new int[resultList.size()] for final return
            int[] result = new int[resultList.size()];
        STEP 17
        Copy all elements in resultList back to result by calling method copyListToArr(result, resultList)
        ...
        -FUNC void constructCountArr(Map<Integer, List<Integer>> hashFreqToValList, int minFreqVal, int[] cacheArr)
        STEP 1
        ...
        If k > valList.size(), (meaning all elements in valList should be added into resultList)
            STEP 2
            Add all eleemnts in valList into resultList
                resultList.addAll(valList);
            STEP 3
            Return (k - valList.size()) as new value of k
                return k - valList.szie();

        Now in the following satisfy k <= valList.size()
        
        STEP 4
        Iterate idx from 0 to k - 1 of valList
            STEP 5
            Add valList.get(idx) into resultList
                resultList.add(valList.get(idx));
        STEP 6
        Return 0 as new value of k
            return 0;
...
class TopKFrequentV1 {
    ...
    public static int[] getTopKFrequent(int[] nums, int k) {
        ...
        System.out.println("countArr: " + Arrays.toString(cacheArr));
        // STEP 8
        List<Integer> resultList = new ArrayList<>();
        // STEP 9
