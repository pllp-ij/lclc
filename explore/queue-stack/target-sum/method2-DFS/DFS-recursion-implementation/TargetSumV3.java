...
/*
    ...
    DESCRIPTION:
        ...
        STEP 1
        If isFound(nums, idxInNums, curValue, target), (meaning the target is found in current level of recursion)
            STEP 2
            Increase first element in result by one
                (result[0])++;
        STEP 3
        If isNotValid(nums, idxInNums), (meaning if curNode is not valid)
            STEP 4
            Directly return
                return;
        STEP 5
        ...
        -FUNC boolean isFound(int[] nums, int idxInNums, int curValue, int target)
        STEP 1
        ...
        STEP 1
        If idxInNums >= nums.length, (meaning the idxInNums for next steps is outside the valid range of nums)
            STEP 2
            Return true
        STEP 3
        Return false;
...
public class TargetSumV3 {
    ...
    public static void DFS(int[] nums, int idxInNums, int curValue, int target, int[] result) {
        // STEP 1
        if (isFound(nums, idxInNums, curValue, target)) {
            // STEP 2
            (result[0])++;
        }
        // STEP 3
    ...
    public static boolean isFound(int[] nums, int idxInNums, int curValue, int target) {
        // STEP 1
        if (curValue == target && idxInNums == nums.length) {
            // STEP 2
            return true;
        }
        // STEP 3
