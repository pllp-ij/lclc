...
/*
    ...
    DESCRIPTION:
        ...
        STEP 1
        If isFound(nums, curNode, target), (meaning the target is found in current level of recursion)
            STEP 2
            Increase first element in result by one
                (result[0])++;
        STEP 3
        If isNotValid(nums, curNode), (meaning if curNode is not valid)
            STEP 4
            Directly return
                return;
        STEP 5
        ...
        STEP 1
        If curNode.val == target and curNode.idxInNums == nums.length, (meaning current value is equal to target and curNode is in lowest level)
            STEP 2
            Return true;
        STEP 3
        Return false;
        ...
        STEP 1
        If curNode.idxInNums >= nums.length, (meaning the idxInNums for next steps is outside the valid range of nums)
            STEP 2
            Return true
        STEP 3
        Return false;
